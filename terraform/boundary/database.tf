...
resource "boundary_host_catalog_static" "db_app" {
  name        = "database"
  description = "MSSSQL database"
  scope_id    = boundary_scope.application.id
}
...
resource "boundary_host_static" "db_app" {
  type            = "static"
  name            = "database"
  description     = "MSSSQL database"
  address         = local.database_url
  host_catalog_id = boundary_host_catalog_static.db_app.id
}
...
resource "boundary_host_set_static" "db_app" {
  type            = "static"
  name            = "database"
  description     = "Host set for MSSQL Database"
  host_catalog_id = boundary_host_catalog_static.db_app.id
  host_ids        = [boundary_host_static.db_app.id]
}
